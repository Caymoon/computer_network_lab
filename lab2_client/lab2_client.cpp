...
-time 从服务器端获取当前时间
-quit 退出客户
-gbn 测试 GBN 协议实现可靠数据传输
[X] [0,1] 模拟数据包丢失的概率 [Y] [0,1] 模拟 ACK 丢失的概率
*/
/****************************************************************/
void printTips() {
	printf("*****************************************\n");
	...
	printf(" 	|-testgbn 0.2 0.2 to test the gbn| \n");
	printf("*****************************************\n");
}
...
int main(int argc, char* argv[]) {
	...
	//使用 -testgbn [X] [Y] 测试 GBN 其中[X]表示数据包丢失概率,[Y]表示 ACK 丢包概率
	printTips();
	int ret;
	...
	while (true) {
		...
		if (!strcmp(cmd, "-time")) {
			sendto(socketClient, "-time", strlen("-itme") + 1, 0, (SOCKADDR*)&addrServer, sizeof(SOCKADDR));
		...
		if (!strcmp(cmd, "-quit")) {
			sendto(socketClient, "-quit", strlen("-quit") + 1, 0, (SOCKADDR*)&addrServer, sizeof(SOCKADDR));
			recvfrom(socketClient, buffer, BUFFER_LENGTH, 0, (SOCKADDR*)&addrServer, &len);
			...
			while (true)
			{
				...
				case 1://等待接收数据阶段
					...
					if (!(waitSeq - seq)) {
						...
						//如果当前一个包都没有收到,则等待 Seq 为 1 的数 据包,不是则不返回 ACK(因为并没有上一个正确的 ACK)
						if (!recvSeq) {
							continue;
						}
						buffer[0] = recvSeq;
